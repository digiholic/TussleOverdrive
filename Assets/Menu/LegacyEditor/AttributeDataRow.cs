...
public class AttributeDataRow : MonoBehaviour, LegacyDataViewer
{
    ...
    public void FighterChanged(FighterInfo info)
    {
        if (variable_name != "")
        {
            ...
            type_label.value = vardata.type.ToString();
            value_label.value = vardata.value;
        }
    }
    ...
    void OnTypeChange(string value)
    {
        ...
        {
            vtype = VarType.FLOAT;
            value_label.validation = UIInput.Validation.Float;
            //value_validator.logic = UIInputValidator.Validation.Float;
        }
        if (value == "INT")
        {
            vtype = VarType.INT;
            value_label.validation = UIInput.Validation.Integer;
            //value_validator.logic = UIInputValidator.Validation.Integer;
        }
        if (value == "BOOL")
        {
            vtype = VarType.BOOL;
            value_label.validation = UIInput.Validation.Alphanumeric;
